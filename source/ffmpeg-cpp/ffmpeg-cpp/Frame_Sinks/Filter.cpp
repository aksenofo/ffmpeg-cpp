...
const char* Filter::GetBufferName(AVMediaType mediaType)
{
    ...
    if (mediaType == AVMEDIA_TYPE_VIDEO)
        ...
        throw new FFmpegException(std::string("Media type ") + av_get_media_type_string(mediaType) + " is not supported by filters.");
}
...
{
    if (!initialized)
        return; // can't close if we were never opened
