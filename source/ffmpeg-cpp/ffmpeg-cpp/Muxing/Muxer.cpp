...
{
    if (opened)
        throw FFmpegException("You cannot open a new stream after something was written to the muxer");
...
{
    if (opened)
        return true; // we were already opened before - always primed from now on!
    bool allPrimed = true;
    for (int i = 0; i < outputStreams.size(); ++i) {
        if (!outputStreams[i]->IsPrimed())
            allPrimed = false;
    }
