...
bool VideoCodec::IsFrameRateSupported(AVRational* frameRate)
{
    if (!codecContext->codec->supported_framerates)
        return true; // all frame rates are fair game
    const AVRational* p = codecContext->codec->supported_framerates;
    while (p->num) {
        if (av_cmp_q(*p, *frameRate) == 0)
            return true;
        p++;
    }
    return false;
...
OpenCodec* VideoCodec::Open(int width, int height, AVRational* frameRate, AVPixelFormat format)
{
    ...
    if (!IsPixelFormatSupported(format))
        throw FFmpegException("Pixel format " + string(av_get_pix_fmt_name(format)) + " is not supported by codec " + codecContext->codec->name);
    if (!IsFrameRateSupported(frameRate))
        throw FFmpegException("Frame rate " + to_string(frameRate->num) + "/" + to_string(frameRate->den) + " is not supported by codec " + codecContext->codec->name);
