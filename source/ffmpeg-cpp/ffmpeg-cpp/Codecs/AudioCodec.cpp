...
bool AudioCodec::IsSampleRateSupported(int sampleRate)
{
    ...
    if (!codecContext->codec->supported_samplerates)
        return true; // all sample rates are fair game
...
OpenCodec* AudioCodec::Open(int bitRate, AVSampleFormat format, int sampleRate)
{
    ...
    if (!IsFormatSupported(format))
        throw FFmpegException("Sample format " + string(av_get_sample_fmt_name(format)) + " is not supported by codec " + codecContext->codec->name);
    if (!IsSampleRateSupported(sampleRate))
        throw FFmpegException("Sample rate " + to_string(sampleRate) + " is not supported by codec " + codecContext->codec->name);

    // if the codec is not an audio codec, we are doing it wrong!
    if (codecContext->codec->type != AVMEDIA_TYPE_AUDIO)
        throw FFmpegException("An audio output stream must be initialized with an audio codec");
